...
function SidebarProvider({
  ...
  // Mobile drawer: close on route change so nav links don't leave the sheet open
  // (centralized here instead of per-link onClick — works for Link, Logo, dropdown items).
  React.useEffect(() => {
    if (!isMobile) return
    setOpenMobile(false)
  }, [pathname, isMobile])
...
function Sidebar({
  ...
  return (
    <div
      className="group peer hidden min-h-0 text-sidebar-foreground md:block"
      data-state={state}
