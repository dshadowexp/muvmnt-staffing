...
export async function claimReferralCode(
  ...
  if (!normalized) {
    if (session) {
      redirect({ href: `/staff?ref_error=invalid`, locale });
    }
    redirect({ href: "/sign-up/staff?ref_error=invalid", locale });
    return;
  }
  ...
  if (session) {
    ...
    redirect({ href: `/staff?${query}`, locale });
    return;
  }
  ...
  redirect({ href: `/sign-up/staff?${signUpParams.toString()}`, locale });
}
