  ...
  return (
    <div className="md:hidden space-y-2">
      <p className="flex items-center gap-2 text-sm font-semibold text-foreground">
        {step && CurrentIcon ? (
          <>
            <span className="flex size-7 shrink-0 items-center justify-center rounded-full border-2 border-primary bg-primary text-primary-foreground">
              <CurrentIcon className="size-3.5" />
            </span>
            <span>{titleFor(step.id, step.title)}</span>
          </>
        ) : (
          <span className="text-muted-foreground">{tKyc("progressFallback")}</span>
        )}
      </p>
