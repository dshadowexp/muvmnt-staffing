...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(data: ForgotPasswordInput) {
    ...
    try {
      ...
      setSuccess(
        `We've sent a password reset link to ${data.email.trim()}. Check your inbox — it may take a minute or two to arrive.`,
      );
    } catch (err) {
      const code = (err as { code?: string })?.code ?? "";
      if (code === "auth/user-not-found") {
        setSuccess(
          `If an account exists for ${data.email.trim()}, a reset link has been sent. Check your inbox.`,
        );
      } else {
