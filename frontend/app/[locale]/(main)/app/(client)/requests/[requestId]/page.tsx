...
export default async function StaffRequestPage({
  ...
  const jobData = getCurrentUser().then(async ({ user }) => {
    ...
    return { staffRequest, options };
  });

  return (
    <div className="container my-4 max-w-5xl space-y-6">
      <BackLink backHref="/app" title="Staff requests" />

      <SuspendedItem
        item={jobData}
        fallback={<JobDetailSkeleton />}
        result={({ staffRequest, options }) => (
          <JobDetailContent staffRequest={staffRequest} options={options} requestId={id} />
        )}
      />
    </div>
  );
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {options
            ...
            <Link
              ...
              href={`/app/staff-request/${requestId}/${option.href}`}
              key={option.href}
            >
