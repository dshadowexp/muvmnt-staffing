...
/** Schedule-only step (wizard / preview) — same refinements as full create */
export const staffRequestScheduleSchema = z
  .object({
    profession: staffRequestFields.profession,
...
/** Wizard step 1: schedule + optional advanced fields (requirements, tasks, positions). */
...
/** Wizard step 3: notes + accepted rate (schedule and staffing details come from step 1). */
export const staffRequestWizardConfirmSchema = z.object({
  notes: z.string().optional(),
  acceptedHourlyRate: z.coerce
    .number({ error: "Enter an hourly rate" })
    .min(MIN_HOURLY, `Hourly rate must be at least $${MIN_HOURLY}`),
});
